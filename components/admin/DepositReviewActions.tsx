...
export function DepositReviewActions({ txId, currentStatus, alreadySettled }: Props) {
  ...
  async function callApi(action: string, extraBody?: object) {
    ...
    setErrorMsg("");
    try {
      const res = await fetch(`/api/admin/deposits/${txId}`, {
        method: "PATCH",
    ...
    return (
      <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
        <p className="text-sm text-emerald-400">
          This deposit has been settled in the ledger and cannot be modified.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-4">
        <p className="text-sm text-zinc-500">
          This deposit is not in a reviewable state ({currentStatus}).
        </p>
      </div>
    );
  }
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5 space-y-4">
      ...
      {state === "idle" && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2">
          <p className="text-xs text-amber-400">
            Approving will immediately credit the user&apos;s ledger account. This cannot be undone.
          </p>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ...
          {state === "loading" ? "Processing…" : "Approve & Credit"}
        </button>
