...
type Props = {
  ...
  t: DashboardDict["realDepositSection"];
  lang: Lang;
};
...
export function RealDepositSection({ depositAddress, walletCount, initialDeposits, t, lang }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-5">
        <h3 className="text-sm font-semibold text-blue-300 mb-3">{t.howItWorksTitle}</h3>
        <ol className="space-y-2 text-sm text-zinc-400">
          ...
          <li className="flex gap-2">
            ...
            {t.step3Prefix} <strong className="text-zinc-200">"{t.step3Action}"</strong> {t.step3Suffix}
          </li>
        </ol>
      ...
      {walletCount === 0 && (
        <div className="flex items-start gap-3 rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
          ...
          <div>
            <p className="text-sm font-medium text-amber-300">{t.noWalletTitle}</p>
      ...
      <div className="flex items-center gap-4">
        ...
        {checkResult && !checkResult.error && (
          ...
          >
            ...
            {checkResult.found > 0
              ? `${checkResult.found} ${t.foundSuffix}`
              : t.noNewDeposits}
          </span>
        )}
      ...
      {deposits.length > 0 && (
        <div>
          <h3 className="text-sm font-medium text-zinc-400 mb-3">{t.recentTitle}</h3>
          <div className="rounded-xl border border-zinc-800 overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-900/80">
                  <th className="text-left px-4 py-3 text-xs text-zinc-500 font-medium">{t.columns.date}</th>
                  <th className="text-right px-4 py-3 text-xs text-zinc-500 font-medium">{t.columns.amount}</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs text-zinc-500 font-medium">{t.columns.status}</th>
                </tr>
              </thead>
                ...
                {deposits.map((d) => (
                  <tr key={d.id} className="hover:bg-zinc-800/30">
                    <td className="px-4 py-3 text-zinc-400 text-xs">
                      {new Date(d.createdAt).toLocaleDateString(dateLocale(lang))}
                    </td>
