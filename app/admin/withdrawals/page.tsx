...
import { getWithdrawalQueue } from "@/lib/admin/queries";
            ...
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-zinc-800 text-left">
                  {["User", "Asset / Amount", "Destination", "Submitted", "Status", "Actions"].map(
                    (h) => (
                      <th
                        key={h}
                ...
                {rows.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="hover:bg-zinc-800/30 transition-colors">
                      ...
                      <td className="px-5 py-3">
                        <p
                          className={`text-sm font-semibold tabular-nums ${meta?.accentClass ?? "text-zinc-200"}`}
                        >
                      ...
                      <td className="px-5 py-3 text-sm text-zinc-400 max-w-[160px] truncate">
                        {row.destination ?? "—"}
                      </td>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-2xl font-bold text-zinc-100">Withdrawal Review Queue</h1>
        {pending.length > 0 && (
      ...
      <Table title="Awaiting Review" rows={pending} emptyText="No pending withdrawals." />
